...
class SyncTest(object):
    ...
    def copy_file_to_disk(self):
        ...
        print("file {} copying".format(self.local_file_path))
    ...
    def task_start(self):
        ...
        #     target=self.add_dirty_task, args=(), daemon=True))
        # thread_list.append(threading.Thread(
        #     target=self.add_dirty_task2, args=(), daemon=True))
        # thread_list.append(threading.Thread(
        #     target=self.clear_dirty_task, args=(), daemon=True))
        # thread_list.append(threading.Thread(
        #     target=self.count_hung_task, args=(), daemon=True))
