...
void init_ofs(int term, char **args, int argc) {
  ...
  if (!initialized) {
    if (argc == 0) {
      ...
      term_color_print(
          term,
          "Usage: initofs <[Service name] | [Service id]> [logic device nr].\n",
          7);
      return;
    }
    ...
    if (argc > 0 && !isnumeric(args[0])) {
      ...
      resolve_cmd.service_name_smo =
          share_mem(DIRECTORY_TASK, args[0], len(args[0]) + 1, READ_PERM);
    ...
    while (get_msg_count(INITOFS_PORT) == 0) {
      reschedule();
    }  // wait for OFS response
...
void mkdevice_cmd(int term, char **args, int argc) {
  ...
  if (argc != 3 || !isnumeric(args[2])) {
    term_color_print(term, invalid_params, 12);
    term_color_print(
        term, "Usage: mkdevice filename Service_name logic_device.\n", 7);
    return;
  }
